...
type HomeScreenSectionItem =
    ...
    | { type: "upcoming"; heading: string; key: "upcoming" }
    | { type: "list-header"; title: string; key: "list-header" }
    | { type: "subscription"; data: Subscription; expanded: boolean; key: string }
    | { type: "footer"; key: "footer" };
...
const HomeHeader = ({ onAddPress }: { onAddPress: () => void }) => (
    <View className="flex-row justify-between items-center mb-8">
        ...
        <Pressable onPress={onAddPress} className="w-10 h-10 rounded-full bg-white border border-slate-200 shadow-sm items-center justify-center active:bg-slate-50">
            <Text className="text-indigo-600 text-2xl font-medium leading-none mt-[-2px]">+</Text>
        </Pressable>
    </View>
);
...
const UpcomingCard = ({ item }: { item: UpcomingSubscription }) => (
    <View className="bg-white/90 rounded-3xl p-5 w-[130px] mr-4 shadow-sm border border-slate-100 items-center overflow-hidden">
        ...
        <Text className="text-sm font-bold text-slate-800 mb-2 text-center" numberOfLines={1}>{item.name}</Text>
        <View className="bg-indigo-50 px-3 py-1.5 rounded-full border border-indigo-100">
            <Text className="text-xs font-bold text-indigo-700">In {item.daysLeft} days</Text>
        </View>
    </View>
);

const ExpandableSubscriptionCard = ({ item, expanded, onToggleExpansion, onRemove, onEdit }: {
    item: Subscription;
    expanded: boolean;
    onToggleExpansion: () => void;
    ...
    return (
        <Pressable onPress={onToggleExpansion} className={`bg-white/95 p-5 rounded-[28px] mb-4 border border-slate-100 active:bg-slate-50 transition-colors shadow-sm ${expanded ? "pb-6" : ""}`}>
            <View className="flex-row items-center">
                <View className="w-14 h-14 rounded-2xl items-center justify-center mr-4" style={{ backgroundColor: serviceColor }}>
                    <Image source={item.icon} className="w-8 h-8" resizeMode="contain" />
                </View>
                <View className="flex-1 justify-center">
                    <Text className="text-base font-black text-slate-900 mb-1" numberOfLines={1}>{item.name}</Text>
                    <View className="flex-row items-center">
                        <StatusBadge status={item.status} />
                    </View>
                </View>
                <View className="items-end justify-center">
                    <Text className="text-lg font-black text-slate-900 tracking-tight">${item.price.toFixed(2)}</Text>
                    <Text className="text-xs font-bold text-slate-400 uppercase mt-0.5">/{item.billing === "Monthly" ? "mo" : "yr"}</Text>
                </View>
            </View>
            ...
            {expanded && (
                <View className="pt-5 mt-5 border-t border-slate-100 border-dashed gap-4">
                    <View className="flex-row justify-between items-center px-1">
                        <Text className="text-sm font-semibold text-slate-500">Category</Text>
                        <Text className="text-sm font-bold text-slate-900">{item.category || "General"}</Text>
                    </View>
                    ...
                    <View className="flex-row gap-3 mt-2">
                        <Pressable
                            ...
                            className="flex-1 bg-rose-50 border border-rose-100 rounded-xl py-3.5 items-center shadow-sm active:bg-rose-100"
                        >
                            <Text className="text-sm font-bold text-rose-600 tracking-wide">Delete</Text>
                        </Pressable>
                        <Pressable
                            onPress={() => onEdit(item)}
                            className="flex-1 bg-indigo-600 rounded-xl py-3.5 items-center shadow-md shadow-indigo-200 active:bg-indigo-700"
                        >
                            <Text className="text-sm font-bold text-white tracking-wide">Edit</Text>
                        </Pressable>
                    </View>
                </View>
            )}
...
export default function HomeScreen() {
    ...
    // Track if we are editing an existing item (null means we are adding a new one)
    const [editingSubId, setEditingSubId] = useState<string | null>(null);

    // Context
    const { subscriptions, upcomingSubscriptions, balance, addSubscription, removeSubscription, updateSubscription } = useSubscriptions();

    const screenData: HomeScreenSectionItem[] = [
        { type: "header", key: "header" },
        { type: "balance", key: "balance" },
        { type: "upcoming", heading: "Upcoming", key: "upcoming" },
        { type: "list-header", title: "All Subscriptions", key: "list-header" },
    ...
    // Opens the modal and populates it with the selected expense's data
    const openEditModal = (sub: Subscription) => {
        setEditingSubId(sub.id);
        setNewName(sub.name);
        setNewPrice(sub.price.toString());
        setNewStatus(sub.status as "active" | "paused" | "cancelled" || "active");
    ...
    const handleSave = () => {
        ...
        if (editingSubId) {
            // EDIT EXISTING
            const oldSub = subscriptions.find(s => s.id === editingSubId);
            if (oldSub) {
                updateSubscription(editingSubId, {
                    ...oldSub, // Keep the old icon, color, etc.
                    name: newName,
                    price: priceNum,
                    status: newStatus,
                });
        ...
        <SafeAreaView className="flex-1 bg-[#F8FAFC]" edges={["top", "left", "right"]}>
            ...
            <FlatList
                ...
                renderItem={({ item }) => {
                    switch (item.type) {
                        case "header":
                            return <HomeHeader onAddPress={() => setAddModalVisible(true)} />;
                        case "balance":
                            return (
                                <View className="bg-white rounded-[32px] p-8 shadow-xl shadow-slate-200/50 border border-slate-100 mb-2 relative overflow-hidden">
                                    <View className="absolute -top-10 -right-10 w-32 h-32 bg-indigo-100 rounded-full blur-3xl opacity-60" />
                                    <Text className="text-sm font-bold text-slate-500 tracking-wider mb-2 uppercase">Total Monthly Spend</Text>
                                    <Text className="text-5xl font-black text-slate-900 tracking-tighter mb-4">{formatCurrency(balance.amount)}</Text>
                                </View>
                            );
                        case "upcoming":
                            return (
                                <View className="mb-2">
                                    ...
                                    <FlatList
                                        ...
                                        showsHorizontalScrollIndicator={false}
                                        contentContainerClassName="py-2"
                                        renderItem={({ item: upcomingItem }) => <UpcomingCard item={upcomingItem} />}
                                    />
                                </View>
                            );
                        case "list-header":
                        ...
                        case "subscription":
                            return (
                                <ExpandableSubscriptionCard
                                    ...
                                    onEdit={openEditModal} // Passed down to the button
                                    onToggleExpansion={() => setExpandedSubId((prev) => (prev === item.data.id ? null : item.data.id))}
                                />
                            );
                        case "footer":
                            return (
                                <View className="items-center mt-12 mb-8 gap-2">
                                    <View className="w-12 h-1 bg-slate-200 rounded-full mb-2" />
                                    <Text className="text-xs font-bold text-slate-400 tracking-wider">SECURELY ENCRYPTED</Text>
                                </View>
                            );
                        default:
                            return null;
                    ...
                    <View className="flex-1 justify-end bg-black/50">
                        <TouchableWithoutFeedback onPress={Keyboard.dismiss}>
                            <KeyboardAvoidingView
                                ...
                                className="bg-white p-6 rounded-t-[32px] shadow-2xl"
                            >
                                <View className="flex-row justify-between items-center mb-6">
                                    {/* Dynamic Title based on adding vs editing */}
                                    <Text className="text-2xl font-black text-slate-900">
                                        {editingSubId ? "Edit Expense" : "Add Expense"}
                                    </Text>
                                    <Pressable onPress={closeModal} className="w-8 h-8 bg-slate-100 rounded-full items-center justify-center">
                                        <Text className="font-bold text-slate-500">X</Text>
                                    </Pressable>
                                </View>

                                {/* Inputs */}
                                <Text className="text-sm font-semibold text-slate-500 mb-2 ml-1">Expense Name</Text>
                                <TextInput
                                    className="bg-slate-50 border border-slate-200 rounded-2xl p-4 mb-4 text-base font-semibold text-slate-900"
                                ...
                                <Text className="text-sm font-semibold text-slate-500 mb-2 ml-1">Amount (RM)</Text>
                                <TextInput
                                    className="bg-slate-50 border border-slate-200 rounded-2xl p-4 mb-6 text-base font-semibold text-slate-900"
                                    placeholder="0.00"
                                ...
                                <Text className="text-sm font-semibold text-slate-500 mb-2 ml-1">Status</Text>
                                ...
                                <Pressable onPress={handleSave} className="bg-indigo-600 py-4 rounded-2xl items-center shadow-md active:bg-indigo-700 mb-6">
                                    {/* Dynamic Button Text based on adding vs editing */}
                                    <Text className="text-white font-bold text-lg">
                                        {editingSubId ? "Update Expense" : "Save Expense"}
                                    </Text>
                                </Pressable>
                            </KeyboardAvoidingView>
                        </TouchableWithoutFeedback>
